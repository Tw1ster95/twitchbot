    ...
    if(message.startsWith(config.prefix)) {
        ...
        if(command === 'followage') {
			if(args.length > 0) {
				...
				displayInfo(client, channel, tags, config);
		}
    }
}

async function displayInfo(client, channel, tags, user, config) {
	await util.api(`https://api.twitch.tv/helix/users/follows?to_id=${config.streamerID}&from_id=${user ? user.id : tags['user-id']}`, async function (err, res, body) {
		if (err) return;
